/**
 * compute-vp9 — VP9 dequantization lookup tables (8-bit depth)
 *
 * Tables from the VP9 specification (libvpx vp9/common/vp9_quant_common.c).
 * qindex 0..255 maps to the actual DC/AC quantizer step used to scale
 * decoded coefficients before the inverse transform.
 */
#pragma once
...
static inline int vp9_clamp_qindex(int qindex)
{
    return qindex < 0 ? 0 : (qindex > 255 ? 255 : qindex);
}

static inline int16_t vp9_dc_quant(int qindex, int delta)
{
    return vp9_dc_qlookup[vp9_clamp_qindex(qindex + delta)];
}

static inline int16_t vp9_ac_quant(int qindex, int delta)
{
    return vp9_ac_qlookup[vp9_clamp_qindex(qindex + delta)];
}
